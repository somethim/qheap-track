...
const props = withDefaults(defineProps<BaseChartProps<T> & {
  ...
  /**
   * Map of field names to their formatted field names
   * Example: { revenue: 'revenueFormatted', expenses: 'expensesFormatted' }
   */
...
<template>
  <div :class="cn('w-full h-[400px] flex flex-col items-end', $attrs.class ?? '')">
    ...
    <VisXYContainer :style="{ height: isMounted ? '100%' : 'auto' }" :margin="{ left: 20, right: 20 }" :data="data">
      ...
      <ChartCrosshair v-if="showTooltip" :colors="colors" :items="legendItems" :index="index" :custom-tooltip="customTooltip" :formatted-fields="formattedFields" />
